...
from database.db import create_session_record, login_history, log_security_event, revoke_session_records, users, utcnow
...
def forgot_password():
    ...
    return jsonify({"message": "If a player account exists, its password has been updated."})
...
def me():
    ...
    if user.get("status") == "suspended":
        session.clear()
        return jsonify({"authenticated": False, "message": "Account suspended."}), 403
